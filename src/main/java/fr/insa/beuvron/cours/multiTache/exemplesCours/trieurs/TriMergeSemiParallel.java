 ...
 * @author francois
 */
public class TriMergeSemiParallel extends Trieur implements Runnable {
    ...
    public static int defaultMinParallel(int tailleTableau) {
        return Math.max(3, tailleTableau/Runtime.getRuntime().availableProcessors());
    }
    ...
    public void run() {
        try {
            ...
            if (max - min < this.minNoumberOfElemsForParallel) {
                TriMergeSequentiel seq = new TriMergeSequentiel(this, min, max);
                this.keepStatus = seq.effectueTrie();
            } else {
                int milieu = (max + min) / 2;
                TriMergeSemiParallel gauche = new TriMergeSemiParallel(this, min, milieu);
                this.declareNewThread();
                Thread bGauche = new Thread(gauche);
                bGauche.start();
                TriMergeSemiParallel droite = new TriMergeSemiParallel(this, milieu, max);
                this.declareNewThread();
            ...
            this.keepStatus = TrieStatus.TOO_MUCH_THREADS;
